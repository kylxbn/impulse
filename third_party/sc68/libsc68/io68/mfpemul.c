 ...
 *
 * Modified for Impulse on 2026-03-24:
 * - avoid aborting the host application when an unsupported MFP timer
 *   mode is requested; fail soft by disabling that timer instead
 *
 */
                 ...
                 int timer, int68_t v, const bogoc68_t bogoc)
{
  ...
  if (timer < TIMER_C) {
    ...
    if (v >= 8) {
      ...
      /* Some tunes poke unsupported MFP modes transiently. Failing soft
         here matches the existing fallback behavior below instead of
         aborting the whole host application in debug builds. */
