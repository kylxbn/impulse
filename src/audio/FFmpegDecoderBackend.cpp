#include "FFmpegDecoderBackend.hpp"
...
#include "metadata/FFmpegMetadataReader.hpp"
...
class FFmpegDecoderProvider final : public DecoderProvider {
public:
...
// ---------------------------------------------------------------------------
FFmpegDecoderBackend::FFmpegDecoderBackend() = default;
...
// ---------------------------------------------------------------------------
DecoderBackend::OpenResult FFmpegDecoderBackend::open(const MediaSource& source,
                                                      int output_sample_rate) {
    close();
    ensureFfmpegNetworkInit();
    ...
    // ---- Read metadata ----
    auto meta = FFmpegMetadataReader::read(source);
    ...
    track_info_.decoder_analysis.push_back(TrackInfoField{
        .label = "Trim mode",
...
void FFmpegDecoderBackend::close() {
...
// ---------------------------------------------------------------------------
bool FFmpegDecoderBackend::initResampler() {
    freeResampler();
...
bool FFmpegDecoderBackend::enableManualSkipExport() {
    if (!codec_ctx_)
        return false;
...
void FFmpegDecoderBackend::freeResampler() {
...
bool FFmpegDecoderBackend::resampleInto(std::vector<float>& buf,
...
// ---------------------------------------------------------------------------
int FFmpegDecoderBackend::decodeNextFrames(std::vector<float>& out_pcm) {
...
// ---------------------------------------------------------------------------
bool FFmpegDecoderBackend::seek(double position_seconds) {
...
void FFmpegDecoderBackend::setReplayGainSettings(ReplayGain::ReplayGainSettings settings) {
    replay_gain_settings_ = settings;
    replay_gain_scale_ = ReplayGain::linearGainForTrack(track_info_, replay_gain_settings_);
}

std::optional<TrackInfo> FFmpegDecoderBackend::consumeTrackInfoUpdate() {
...
bool FFmpegDecoderBackend::refreshLiveTrackInfo() {
...
const DecoderProvider& ffmpegDecoderProvider() {
    static const FFmpegDecoderProvider kProvider;
    return kProvider;
}
