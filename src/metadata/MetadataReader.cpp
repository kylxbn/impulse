...
MetadataReader::read(const MediaSource& source,
    ...
    appendField(info.ffmpeg_analysis, "Source", input);
    appendIntegerField(info.ffmpeg_analysis, "File size (bytes)", info.file_size_bytes);
    appendField(info.ffmpeg_analysis, "Container duration",
                info.duration_seconds > 0.0 ? std::format("{:.6f} s", info.duration_seconds) : "");
    appendIntegerField(info.ffmpeg_analysis, "Container bitrate (bps)", fmt_ctx->bit_rate);

    // Container format name
    if (fmt_ctx->iformat && fmt_ctx->iformat->name) {
        info.container_format = fmt_ctx->iformat->name;
        appendField(info.ffmpeg_analysis, "Container short name", fmt_ctx->iformat->name);
        if (fmt_ctx->iformat->long_name)
            appendField(info.ffmpeg_analysis, "Container long name", fmt_ctx->iformat->long_name);
    }
    appendIntegerField(info.ffmpeg_analysis, "Stream count", fmt_ctx->nb_streams, true);
    ...
    if (audio_idx >= 0) {
        ...
        if (codec) {
            ...
            appendField(info.ffmpeg_analysis, "Codec short name", codec->name);
            if (codec->long_name)
                appendField(info.ffmpeg_analysis, "Codec long name", codec->long_name);
        }
        if (const AVCodecDescriptor* descriptor = avcodec_descriptor_get(par->codec_id)) {
            appendField(info.ffmpeg_analysis, "Codec descriptor", descriptor->name);
            if (descriptor->long_name)
                appendField(info.ffmpeg_analysis, "Codec description", descriptor->long_name);
        }
        ...
        appendIntegerField(info.ffmpeg_analysis, "Audio stream index", audio_idx, true);
        appendIntegerField(info.ffmpeg_analysis, "Audio stream id", stream->id);
        appendIntegerField(info.ffmpeg_analysis, "Stream sample rate", par->sample_rate);
        appendIntegerField(info.ffmpeg_analysis, "Channels", par->ch_layout.nb_channels);
        appendIntegerField(info.ffmpeg_analysis, "Bits per raw sample", par->bits_per_raw_sample);
        appendIntegerField(info.ffmpeg_analysis, "Bits per coded sample", par->bits_per_coded_sample);
        appendIntegerField(info.ffmpeg_analysis, "Stream bitrate (bps)", par->bit_rate);
        appendIntegerField(info.ffmpeg_analysis, "Frame size", par->frame_size);
        appendIntegerField(info.ffmpeg_analysis, "Block align", par->block_align);
        appendIntegerField(info.ffmpeg_analysis, "Initial padding (samples)", par->initial_padding, true);
        appendIntegerField(info.ffmpeg_analysis, "Trailing padding (samples)", par->trailing_padding, true);
        appendIntegerField(info.ffmpeg_analysis, "Seek preroll (samples)", par->seek_preroll, true);
        appendIntegerField(info.ffmpeg_analysis, "Extradata size (bytes)", par->extradata_size);
        appendIntegerField(info.ffmpeg_analysis, "Profile", par->profile);
        if (stream->duration != AV_NOPTS_VALUE) {
            appendIntegerField(info.ffmpeg_analysis, "Stream duration (ticks)", stream->duration, true);
            appendField(info.ffmpeg_analysis, "Stream duration",
                        std::format("{:.6f} s", stream->duration * av_q2d(stream->time_base)));
        }
        if (stream->start_time != AV_NOPTS_VALUE) {
            appendIntegerField(info.ffmpeg_analysis, "Stream start time (ticks)", stream->start_time, true);
            appendField(info.ffmpeg_analysis, "Stream start time",
                        std::format("{:.6f} s", stream->start_time * av_q2d(stream->time_base)));
        }
        appendField(info.ffmpeg_analysis, "Stream time base",
                    std::format("{}/{}", stream->time_base.num, stream->time_base.den));
        appendIntegerField(info.ffmpeg_analysis, "Reported frame count", stream->nb_frames);
        if (par->format >= 0) {
            if (const char* sample_fmt = av_get_sample_fmt_name(static_cast<AVSampleFormat>(par->format)))
                appendField(info.ffmpeg_analysis, "Sample format", sample_fmt);
        }
        const char* profile_name = avcodec_profile_name(par->codec_id, par->profile);
        if (profile_name)
            appendField(info.ffmpeg_analysis, "Profile name", profile_name);

        // Channel layout
        char ch_buf[64]{};
        av_channel_layout_describe(&par->ch_layout, ch_buf, sizeof(ch_buf));
        info.channel_layout = ch_buf;
        appendField(info.ffmpeg_analysis, "Channel layout", info.channel_layout);
...
// ---------------------------------------------------------------------------
// MetadataReader::parseTags
// ---------------------------------------------------------------------------

void MetadataReader::parseTags(AVDictionary* dict, TrackInfo& info) {
    if (!dict) return;
...
// ---------------------------------------------------------------------------
// MetadataReader::parseReplayGain
// ---------------------------------------------------------------------------

void MetadataReader::parseReplayGain(AVDictionary* stream_dict,
...
// ---------------------------------------------------------------------------
// MetadataReader::decodeAlbumArt
// ---------------------------------------------------------------------------

bool MetadataReader::decodeAlbumArt(AVFormatContext* fmt_ctx, TrackInfo& info) {
